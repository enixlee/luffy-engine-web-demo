<template>
  <div class="UserAdd">
    <div class="container">
      <common-input
        showAsterisk
        :labelText="getLang('LABEL_REAL_NAME')"
        :hintText="getLang('PLACEHOLDER_EMPLOYEE_NAME')"
        v-model="fullName"
        :errorText="getLang('ERROR_INPUT_FULL_NAME')"
        :showErrorText="showFullNameErrorText"
        :handleBlur="handleFullName"
        :handleFocus="handleFullNameFocus"
      ></common-input>
      <common-input
        showAsterisk
        inputType="number"
        :labelText="getLang('LABEL_CELLPHONE')"
        :hintText="getLang('PLACEHOLDER_PHONE_RETRIEVE_PASSWORD')"
        :errorText="getLang('ERROR_PHONE_NUMBER_INPUT')"
        :showErrorText="showPhoneErrorText"
        v-model="cellPhone"
        :handleBlur="handleIphone"
        :handleFocus="handleIphoneFocus"
      ></common-input>
      <common-input
        showAsterisk
        :labelText="getLang('LABEL_USER_NAME')"
        :hintText="getLang('PLACEHOLDER_USERNAME_RULE')"
        :errorText="getLang('ERROR_LOGIN_USERNAME_INPUT')"
        :showErrorText="showUsernameErrorText"
        v-model="userNameText"
        :handleBlur="handleUserNameText"
        :handleFocus="handleUserNameFocus"
      ></common-input>
      <common-input
        showAsterisk
        inputType="password"
        :labelText="getLang('LABEL_PASSWORD')"
        :hintText="getLang('PLACEHOLDER_PASSWORD_INPUT')"
        :errorText="getLang('ERROR_LOGIN_PASSWORD_INPUT')"
        :showErrorText="showPasswordErrorText"
        v-model="passwordText"
        :handleBlur="handlePasswordErrorText"
        :handleFocus="handlePasswordFocus"
      ></common-input>
      <common-input
        showAsterisk
        inputType="password"
        :labelText="getLang('LABEL_CONFIRM_PASSWORD')"
        :hintText="getLang('INPUT_PLACEHOLDER_RETYPE_PASSWORD')"
        :errorText="getLang('ERROR_ADD_USER_RETYPE_PASSWORD_INPUT')"
        :showErrorText="showAgainPasswordErrorText"
        v-model="confirmPasswordText"
        :handleBlur="handleAgainPasswordErrorText"
        :handleFocus="handleAgainPasswordFocus"
      >
      </common-input>
      <common-input
        showAsterisk
        :labelText="getLang('LABEL_COMPANY')"
      >
        <drop-down-search
          slot="customInput"
          :hintText="getLang('PLACEHOLDER_COMPANY_NAME')"
          :formatValue="formatValue"
          :subject="storeTypeQuerySubject"
          :filterLoaded="filterStoreTypeLoaded"
          :queryKey="'storeName'"
          :paramsExtends="storeTypeQueryParams"
          @choose="chooseItem"
        >
        </drop-down-search>
      </common-input>
      <common-input
        showAsterisk
        :labelText="getLang('LABEL_USER_ROLE')"
      >
        <mu-select-field
          slot="customInput"
          fullWidth
          multiple
          class="select-drop-down"
          :maxHeight="maxHeight"
          v-model="rolesId">
          <mu-menu-item
            v-for="role in roles"
            :key="role.get('ID')"
            :value="role.get('ID')"
            :title="role.get('ROLE_NAME')"
          ></mu-menu-item>
        </mu-select-field>
      </common-input>
      <div class="notice">
        <p class="notice-one">{{$t('REMINDER_JURISDICTION_INFO')}}</p>
      </div>
      <mu-raised-button
        :label="getLang('BTN_CONFIRM_ADD')"
        class="user-add-btn" :disabled="!confirmBtnDisabled"
        primary
        @click="handleAddUser">
      </mu-raised-button>
    </div>
  </div>

</template>

<script type="text/javascript">
  import {COMPANY_QUERY, USER_ADD, USER_QUERY} from '../../store/Subjects.js';
  import RoleState from '../../store/modules/Role';
  export default {
    name: 'UserAdd',
    components: {},
    props: {},
    data () {
      return {
        fullName: '',
        cellPhone: null,
        showPhoneErrorText: false,
        showUsernameErrorText: false,
        showPasswordErrorText: false,
        showFullNameErrorText: false,
        userNameText: '',
        passwordText: '',
        storeNo: '',
        roles: [],
        roleTextValue: '',
        rolesId: [],
        maxHeight: 300,
        confirmPasswordText: '',
        isValidCompany: false,
        showAgainPasswordErrorText: false,
        company: '',
        totalPageCount: 0,
        storeTypeQuerySubject: COMPANY_QUERY,
        storeTypeQueryParams: {status: JSON.stringify([this.$constants.global.MINIPAY_STATUS_NORMAL])}
      };
    },
    computed: {
      confirmBtnDisabled: function () {
        return this.trimString(this.fullName).length > 0 &&
          this.getPlugin('TypeCheck').isCellphone(this.cellPhone) &&
          this.$validator.userNameValid(this.userNameText) &&
          this.$validator.passwordValid(this.passwordText) &&
          (this.passwordText === this.confirmPasswordText) &&
          this.isValidCompany && this.rolesId.length > 0;
      }
    },
    methods: {
      dataInit: function () {
        this.roles = this.getModuleGetterByName('getValidRoles');
        this.rolesId = [this.roles[0].get('ID')];
      },
      getRolesCn (roleList) {
        let names = '';
        let that = this;
        this.getPlugin('lodash').map(roleList, function (v) {
          let role = RoleState.getRoleByRoleId(that.getStoreState('Role'), v);
          if (role) {
            names += `${role.get('ROLE_NAME')}、`;
          }
        });
        return names.substring(0, names.length - 1);
      },
      formatValue (item) {
        if (this.getPlugin('lodash').isObject(item)) {
          return `[${item.get('STORE_NO')}] ${item.get('STORE_NAME')}`;
        }
        return '';
      },
      filterStoreTypeLoaded (payload) {
        return payload.list;
      },
      chooseItem (item) {
        if (this.getPlugin('lodash').isObject(item)) {
          this.storeNo = item.get('STORE_NO');
          this.company = item.get('STORE_NAME');
          this.isValidCompany = true;
        } else {
          this.isValidCompany = false;
        }
      },
      handleAddUser () {
        this.$store.dispatch(USER_QUERY, {
          userName: this.userNameText,
          pageIndex: 1,
          pageCount: 1
        });
      },
      checkUser: function (payload) {
        let user = payload.list[0];
        if (!user) {
          // 用户名没有重复
          this.roleTextValue = this.getRolesCn(this.rolesId);
          this.showCustomDialog(
            'UserAdd',
            this.getLang('DIALOG_TITLE_USER_ADD'),
            this.getLang('BTN_CONFIRM_ADD'),
            null,
            this.clickReconfirm,
            null,
            {
              fullName: this.fullName,
              cellPhone: this.cellPhone,
              userNameText: this.userNameText,
              company: this.company,
              roleTextValue: this.roleTextValue
            }
          );
        } else {
          this.showDialog(this.getLang('INFO_USER_REPEAT'), this.getLang('BTN_CONFIRM'));
        }
      },
      clickReconfirm () {
        this.$store.dispatch(USER_ADD, {
          userName: this.userNameText,
          password: this.getPlugin('Crypto').MD5(this.passwordText),
          name: this.fullName,
          cellphone: this.cellPhone,
          storeNo: this.storeNo,
          role: JSON.stringify(this.rolesId.map((item) => (parseInt(item))))
        });
      },
      getUserAddResult () {
        this.showToast(this.getLang('TOAST_USER_ADD_SUCCESS'));
      },
      handleFullName () {
        this.showFullNameErrorText = !this.trimString(this.fullName).length > 0;
      },
      handleFullNameFocus () {
        this.showFullNameErrorText = false;
      },
      handleIphoneFocus () {
        this.showPhoneErrorText = false;
      },
      handleIphone () {
        this.showPhoneErrorText = !this.getPlugin('TypeCheck').isCellphone(this.cellPhone);
      },
      handleUserNameText () {
        this.showUsernameErrorText = !this.$validator.userNameValid(this.userNameText);
      },
      handleUserNameFocus () {
        this.showUsernameErrorText = false;
      },
      handlePasswordErrorText () {
        this.showPasswordErrorText = !this.$validator.passwordValid(this.passwordText);
      },
      handlePasswordFocus () {
        this.showPasswordErrorText = false;
      },
      handleAgainPasswordErrorText () {
        this.showAgainPasswordErrorText = !(this.passwordText === this.confirmPasswordText);
      },
      handleAgainPasswordFocus () {
        this.showAgainPasswordErrorText = false;
      }
    },
    watch: {
      passwordText (newVal, oldVal) {
        if (newVal !== oldVal && this.confirmPasswordText.length > 0) {
          this.handleAgainPasswordErrorText();
        }
      }
    },
    created: function () {
      if (this.getModuleGetterByName('isDataLoadComplete')) {
        this.dataInit();
      } else {
        this.Bus(this).on(this.getConstByKey(this, 'events', 'EVENT_APP_DATA_LOADED'), this.dataInit);
      }
      this.$bus.on(USER_ADD, this.getUserAddResult);
      this.$bus.on(USER_QUERY, this.checkUser);
    },
    beforeDestroy: function () {
      this.Bus(this).off(this.getConstByKey(this, 'events', 'EVENT_APP_DATA_LOADED'), this.dataInit);
      this.$bus.off(USER_ADD, this.getUserAddResult);
      this.$bus.off(USER_QUERY, this.checkUser);
    }
  };
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style lang="less" scoped>
  @import "../../assets/common_variable.less";

  .UserAdd {
    width: 100%;
    color: @color-7e8e97;
    .container {
      position: relative;
      padding-top: 20px;
      margin-left: 20px;
      width: 450px;
      .notice {
        color: @color-999999;
        margin: 20px 0 0 58px;
        font-size: 12px;
        .notice-two {
          margin: 10px 0 0 20px;
        }
      }
      .user-add-btn {
        margin: 42px 0 0 350px;
      }
    }

  }
</style>
